test_that("`eval_conditions()` evaluates conditions correctly", {
  ...
  expect_identical(
    sdtm.oak:::eval_conditions(df, x > w, .env = list(w = 3L)),
    c(FALSE, FALSE, NA, TRUE, TRUE)
  )
  expect_identical(
    sdtm.oak:::eval_conditions(df, x > w, .env = tibble::tibble(w = 4L)),
    c(FALSE, FALSE, NA, FALSE, TRUE)
  )
})
