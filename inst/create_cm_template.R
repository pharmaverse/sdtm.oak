...
cm <- MD1 |>
  ...
  assign_no_ct(raw_dataset = MD1, #This is added for pseudocode. Not required as pipe will send it
               raw_variable = MDRAW,
               target_sdtm_var = CMTRT) |>
  #Derive qualifier CMDOSU
  #Use merge and add the qualifier to the topic variable
  assign_ct(raw_dataset = MD1,
            raw_variable = DOSU,
 ...
 # DERIVE CMINDC
  ...
  dplyr::mutate(STUDYID = "test_study",
                DOMAIN = "CM",
                CMCAT = "GENERAL CONMED") |>
  derive_usubjid() |>
  derive_sequence(keys = c(USUBJID,CMTRT)) |>
  derive_visit_visitnum() |>
  derive_study_day(var_in=CMSTDTC,
                   target_var=CMSTDY) |>
  derive_study_day(var_in=CMENDTC,
                   target_var=CMENDY)
